...
def test_request_prefixes_snapshot():
    assert turn_protocol.request("SNAP", "make a box") == (
        "[document snapshot]\nSNAP\n\n[request]\nmake a box")
...
def test_execution_body_unchanged_document():
    ...
    body = turn_protocol.execution_body(
        result, {"objects": {}}, {"objects": {}}, "NEW", [], settings)
    assert body == "[executed OK]\n[document unchanged]\n"


def test_execution_body_with_change_emits_diff_and_snapshot():
    settings = _settings(enhanced_validation=False)
    result = ExecResult(ok=True, output="", error="")
    before = {"objects": {}}
    after = {"objects": {"Box": {"type": "Part::Box"}}}
    body = turn_protocol.execution_body(
        result, before, after, "NEWSNAP", ["Box"], settings)
    assert "[document diff]\n" in body
    assert body.endswith("[new snapshot]\nNEWSNAP")


def test_workflow_tail_renders_warnings_and_ledger():
